    ...
    public void menuSelected(MenuItem selectedItem) {
        ...
        if (usersItem.equals(selectedItem)) {
            ...
            if (Constants.dbUseCloudFoundryDatabase) {
                // CloudFoundry is in beta, so we need to disable some features
                // to avoid problems
                // let's create a CRUD with the helper class CrudBuilder (you
                // can also instantiate a crud directly)
                crud = new CrudBuilder<User>(User.class).setShowDeleteButton(false).setShowUpdateButton(false).build();
            } else {
                // let's create a CRUD instantiating it directly (you can also
                // use the CrudBuilder helper class)
            ...
            CrudComponent<Audit> crud = new CrudBuilder<Audit>(Audit.class).setShowForm(false).build();
            ...
            FilesViewerComponent filesViewerComponent =
                new FilesViewerComponent(Utils.getServerLogsDirectory(), false, true);
